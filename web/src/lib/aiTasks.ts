...
Chỉ trả JSON: {"wantProduct": bool, "keyword": string, "reason": string ngắn}.`
  ...
  analyzeGroups: {
    build: (a) => {
      ...
      const system = goal
        ? `Bạn LỌC danh sách nhóm Facebook theo MỤC TIÊU người dùng mô tả dưới đây. Chấm 0-100 mỗi nhóm theo độ KHỚP với các mục tiêu đó.
...
- CAO (70-100): tên/chủ đề nhóm KHỚP RÕ với MỘT trong các mục tiêu người dùng liệt kê.
- TRUNG BÌNH (40-69): liên quan một phần tới mục tiêu.
- THẤP (0-39): không khớp mục tiêu nào.
Gắn "niche" = nhãn NGẮN cho biết nhóm khớp mục tiêu nào (vd người dùng ghi "nhóm tiếng trung, nhóm AI, rải link aff" → niche là "tiếng Trung" / "AI" / "affiliate/mua sắm").
